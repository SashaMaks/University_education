...
public class Main {
    ...
    private static void Task1() {
        //FIX_ME: проверка правильности ввода оценок студента происходит вне класса студент
        /*System.out.println("Введите 1 оценку для студента: ");
        int grade1 = Checks.readIntInRange(2,5);
        System.out.println("Введите 2 оценку для студента: ");
        int grade2 = Checks.readIntInRange(2,5);
        System.out.println("Введите 3 оценку для студента: ");
        int grade3 = Checks.readIntInRange(2,5);
        int[] grades = new int[3];
        grades[0] = grade1;
        grades[1] = grade2;
        grades[2] = grade3;
        Student Vova = new Student("Вова", grades);*/
        System.out.println("Сколько оценок вы будете вводить?");
        int count = Checks.readInt();
        Student student_vova = new Student("Вова",count);
        for (int i = 0; i < count; i++) {
            System.out.println("Введите " + (i+1) + " оценку:");
            student_vova.setGrade(i,Checks.readInt());
        }

        System.out.println(student_vova.toString());
    }

    // ----------------------------=2=------------------------------------------

    private static void Task2() {
        //FIX_ME: ввод производится не совсем корректно
        ...
        String name, number;
        PhoneDirectory book = new PhoneDirectory();
        System.out.println("Сколько пользователей вы хотите внести в справочник");
        int count = Checks.readInt();
        for (int i = 0; i < count; i++) {
            System.out.println("Введите имя: ");
            name = Checks.readNonEmptyString();
            System.out.println("Введите номер ");
            number = Checks.readValidPhone();
            book.addContact(name,number);
        }
        System.out.println(book.toString());
        ...
        //FIX_ME: проверка ввода проверяет любую строку
        //String phoneToFind = Checks.readNonEmptyString();
        String phoneToFind = Checks.readValidPhone();
    ...
    private static void Task53() {
        //FIX_ME: показ корректности ввода не корректна, так как показывается только данное разработчиком
        /*System.out.println("\nКруг1: \n");
        System.out.println("Введите X: ");
        ...
        System.out.println("Сколько фигур вы хотите создать?");
        int count = Checks.readInt();
        Shape[] shapes = new Shape[count];
        for (int i = 0; i < count; i++) {
            System.out.println("Выберете фигуру для ввода (1-круг, 2-треугольник, 3-квадрат, 4-прямоугольник)");
            String choice = scanner.nextLine();
            switch (choice) {
                case "1":
                    System.out.println("\nКруг:");
        ...
        System.out.println("\nПлощади: \n");
        double totalArea = 0;
        //FIX_ME: требуется создавать раньше, перед вводом фигур
