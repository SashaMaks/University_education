...
void my_file(PNode &p) {
  int n,k;
  std::ifstream file("C:\\Users\\Public\\Documents\\2sem\\Practice 21\\3\\input.txt");
...
// Дополнительная функция: находит минимальный элемент (самый левый узел)
...
// Дополнительная функция: находит следующий по порядку узел
PNode findSecondMax(PNode root) {
  ...
  if (current->left != nullptr) return findMax(current->left);
